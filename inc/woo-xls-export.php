...
class Woo_xls_export {
	...
	public function __construct(){	
		add_action( 'admin_menu', array( $this, 'viwxe_add_menu_link' ) );
		add_action( 'wp_ajax_save_product', 'viwxe_wxe_save_woo_products' );
		add_action( 'wp_ajax_save_category', 'viwxe_wxe_save_woo_category' );
		add_action( 'wp_ajax_save_tags', 'viwxe_wxe_save_woo_tags' );
		add_action( 'wp_ajax_save_orders', 'viwxe_wxe_save_woo_orders' );
		add_action( 'wp_ajax_save_customers', 'viwxe_wxe_save_woo_customers' );
	}
	
	public function viwxe_add_menu_link() {
		$this->page = add_submenu_page(
			...
			'manage_woocommerce',
			'wc_xls_export',
			array( $this, 'viwxe_settings_page' )
		);
	}
	
	public function viwxe_settings_page( $tab = false ){
		ini_set('max_execution_time', 3000);
		$this->viwxe_wxe_set_archive_table();
		$show_settings = ''; $show_archive='';$show_export='';
		if( $this->viwxe_wxe_generate_data_for_export())
			$tab = 'archive';
		...
		$html ='';
		$html .=   '<link href="'.VIVA_WOO_XLS_PATH.'css/style.css" rel="stylesheet" />';
		$html .=   '<link href="'.VIVA_WOO_XLS_PATH.'css/colorbox.css" rel="stylesheet" />';
		$html .=   '<script type="text/javascript">var admin_url = "'.admin_url('admin-ajax.php').'";var pluginpath = "'.VIVA_WOO_XLS_PATH.'";</script>';
		...
		$tab_html = $this->viwxe_get_tab_data( $tab );
		$html .= $tab_html;
		...
		echo $html;
		echo '<script src="'.VIVA_WOO_XLS_PATH.'js/custom.js"></script>';
		echo '<script src="'.VIVA_WOO_XLS_PATH.'js/jquery.bpopup.min.js"></script>';
		echo '<script src="'.VIVA_WOO_XLS_PATH.'js/jquery.colorbox.js"></script>';
	}
	...
	function viwxe_wxe_set_archive_table(){
		global $wpdb;
	...
	function viwxe_get_tab_data( $tab )
	{
		$output_data = '';
		...
		if( ! empty( $tab ) )
		{
			if(  $tab == 'export' )
			{
				$output_data = $this->viwxe_get_export_tab_data();
			}
			else if(  $tab == 'archive' )
			{
				$output_data = $this->viwxe_get_archive_tab_data();
			}	
			else if(  $tab == 'settings' )
			{
				$output_data = $this->viwxe_wxe_get_setting_tab_data();
			}	
		}
	...
	function viwxe_get_export_tab_data(){
		$data = '
			<div class="overview-left">

	<h3><a href="'.VIVA_WOO_XLS_EXPORT_ADMIN_URL.'&amp;tab=export">Export</a></h3>
	<p>Export store details out of WooCommerce into a Execel Spreadsheet file.</p>
	<ul class="ul-disc">
		<li>
			<a class="button-primary all_export_btn export_btn" href="'.VIVA_WOO_XLS_EXPORT_ADMIN_URL.'&amp;tab=export&export=all_products">Export All Products</a>
			<div class="all_product_exporting_loader">
				<img src="'.VIVA_WOO_XLS_PATH.'img/animated_loading.gif" /><span>Exporting Please Wait ... </span>
			</div>
		</li>
		<li>
			<a class="button-primary  cat_export_btn export_btn" href="'.VIVA_WOO_XLS_EXPORT_ADMIN_URL.'&amp;tab=export&export=selected_category">Export Category Products</a>
			...
			<div class="category_settings">
				'.$this->viwxe_wxe_get_all_woo_categories().'
			<div>
		</li>
	</ul>
</div>
	...
	function viwxe_get_archive_tab_data(){
		global $wpdb;
		if( isset( $_REQUEST['action'] ) )
		{
			if( $_REQUEST['action'] == 'delete' )
			{
				...
				if( ! empty( $id ) )
				{
					...
					unlink ( VIVA_WOO_XLS_EXPORT_PATH.'/'. $name . ".xlsx");
		...
		{
			$file_url =   VIVA_WOO_XLS_EXPORT_PATH.'/'.$arch->archive_name.'.xlsx' ;
			$size = $this->viwxe_wxe_formatSizeUnits( filesize ( $file_url ) );
			
			$data .= '
				<tr id="post-404" class="author-self status-inherit" valign="top">
				<td class="column-icon media-icon">
					<img width="48" height="64" src="'.VIVA_WOO_XLS_PATH.'img/excelimg.png" class="attachment-80x60" alt="woo-export_products-2014_05_07.csv">				</td>
				<td class="post-title page-title column-title">
					<strong>'.$arch->archive_name.'.xlsx</strong>
					<div class="row-actions">
						<span class="view"><a href="'.VIVA_WOO_XLS_DOWNLOAD_PATH.$arch->archive_name.'.xlsx" title="download">Download</a></span> | 
						<span class="trash"><a href="'.VIVA_WOO_XLS_EXPORT_ADMIN_URL.'&action=delete&tab=archive&archid='.$arch->archive_id.'&arch_name='.$arch->archive_name.'" title="Delete Permanently">Delete</a></span>
					</div>
				</td>
	...
	function viwxe_wxe_get_all_woo_products(){
		$args = array( 
				'post_type' => 'product', 
				'posts_per_page' => 1000, 
		);
		...
		if( $query->have_posts() ){
			...
			while ( $query->have_posts() ) {
				...
				if( !empty( $db_checked_products ) )
				{
					if( in_array( $query->post->ID, $db_checked_products ) )
						$data .= '<img class="checkimg" src="'.VIVA_WOO_XLS_PATH.'img/checked.png" />
							<input class="checkbox checkeddone"  type="checkbox" name="checked_product[]" value = "'.$query->post->ID.'" checked="checked" /></div>';
					else
						$data .= '<img class="checkimg" src="'.VIVA_WOO_XLS_PATH.'img/unchecked.png" />
			...
			</form><div class="save_loader"><img src="'.VIVA_WOO_XLS_PATH.'/img/loading.gif" alt="" /><span>Saving...</span></div>';
		}
		else
		{
	...
	function viwxe_wxe_get_all_woo_categories(){
		...
		{	
			...
			<div class="save_loader"><img src="'.VIVA_WOO_XLS_PATH.'/img/loading.gif" alt="" /><span>Saving...</span></div>';
		}
		else
	...
	function viwxe_wxe_get_setting_tab_data(){
	...
	function viwxe_wxe_generate_data_for_export(){
		if( isset( $_REQUEST['export']) )
		{
			if( $_REQUEST['export'] == 'all_products' )
			{
				...
				if( $the_query->have_posts() )
				{
					...
					$export_xls_name = $this->viwxe_get_archive_name();
					$records = $full_data;
	...
	function viwxe_get_archive_name(){
		$prefix_name = get_option( 'wxe_archive_prefix' );
	...
	function viwxe_wxe_formatSizeUnits($bytes)
    {
        if ($bytes >= 1073741824)
        {
